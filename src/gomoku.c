...
//BMP图片文件存储位置
//主界面
#define MENU_PATH "/mnt/image/gomoku/menu.bmp"
//棋盘背景
#define BACK_PATH "/mnt/image/gomoku/background.bmp"
    ...
    {   
        ...
        {   
			...
			if (is_win(chessboard,SIZE,SIZE) == BLACK) 
            {
				printf("blackwin\n");
				blackwin++;
				bmp_draw(300,180,"/mnt/image/gomoku/black.bmp");
				sleep(3);
				return;
			}
		} 
        ...
        {                            
			...
			if (is_win(chessboard,SIZE,SIZE) == WHITE) 
            {
				printf("whitewin\n");
				whitewin++;
				bmp_draw(300,180,"/mnt/image/gomoku/white.bmp");
				sleep(3);
				return;
			}
		}
		...
		{
			printf("full\n");
			bmp_draw(300,180,"/mnt/image/gomoku/peace.bmp");
			sleep(3);
			return;
		}	
	}
...
void quit()
{
	bmp_draw(0,0,"/mnt/image/gomoku/quit.bmp");
	sleep(1);
	close(lcd_id);
	lcd_id=-1;
...
void options()
{
	...
	{
		bmp_draw(0,0,"/mnt/image/gomoku/nogame.bmp");
		sleep(3);
	}
	else
	{
		...
		{
			bmp_draw(0,0,"/mnt/image/gomoku/blackwin.bmp");
			sleep(3);
		}
		else
		{
			bmp_draw(0,0,"/mnt/image/gomoku/whitewin.bmp");
			sleep(3);
		}
	}
	return;
